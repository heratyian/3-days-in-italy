...
def repository(tmp_path, monkeypatch):
    records = [
        ...
        {"id": "near-b", "name": "Near B", "latitude": 0, "longitude": 0.01,
         "tags": ["outdoors"], "type": "park", "rating": 5},
        {"id": "near-a", "name": "Near A", "latitude": 0, "longitude": 0.01,
         "tags": ["local_favorite"], "type": "park", "rating": 1},
        {"id": "far", "name": "Far", "latitude": 0, "longitude": 1,
         "tags": ["outdoors"], "type": "museum", "rating": 5},
...
def test_nearby_sorts_by_distance_then_id_and_excludes_origin_and_missing(repository):
    ...
    assert [result.place.id for result in repository.nearby("origin", limit=2)] == ["same", "near-a"]
...
def test_nearby_filters_before_limiting_with_search_semantics(repository):
    results = repository.nearby("origin", tags=["LOCAL FAVORITE", "outdoors"], types=["PARK"], limit=1)
...
@pytest.mark.parametrize("arguments", [
    {"radius_km": -1}, {"radius_km": float("nan")}, {"radius_km": float("inf")}, {"limit": -1},
])
...
@pytest.mark.parametrize("tool, arguments, error", [
    (calculate_distance, {"origin_place_id": "unknown", "destination_place_id": "origin"}, "Unknown place ID"),
    (calculate_distance, {"origin_place_id": "origin", "destination_place_id": "missing"}, "Missing coordinates"),
...
@pytest.mark.parametrize("arguments", [
    {"radius_km": -1}, {"radius_km": float("nan")}, {"radius_km": float("inf")}, {"limit": 0},
])
...
def test_tools_return_authoritative_places_and_explicit_distance_units(repository):
    ...
    distance = calculate_distance.invoke({"origin_place_id": "origin", "destination_place_id": "near-a"})
    assert distance == {
        "origin_place_id": "origin", "destination_place_id": "near-a",
        "distance_km": results[0]["distance_km"], "method": "haversine",
    }
    ...
    def generate(self, messages, **kwargs):
        ...
        if latest.type == "human":
            response = AIMessage(content="", tool_calls=[{
                "id": "nearby", "name": "find_nearby_places",
                "args": {"place_id": "place_001", "radius_km": 10, "limit": 1},
            }])
        elif latest.name == "find_nearby_places":
            candidate = json.loads(latest.content)[0]
            response = AIMessage(content="", tool_calls=[{
                "id": "distance", "name": "calculate_distance", "args": {
                    "origin_place_id": "place_001", "destination_place_id": candidate["place"]["id"],
                },
            }])
        else:
            distance = json.loads(latest.content)
            response = AIMessage(content=f"The alternative is {distance['distance_km']:.1f} km in a straight line.")
        return ChatResult(generations=[ChatGeneration(message=response)])

    monkeypatch.setattr(ChatOpenAI, "_generate", generate)
    result = graph.invoke({"messages": [{"role": "user", "content": "Find an alternative near place_001."}]})
    tool_results = [message for message in result["messages"] if isinstance(message, ToolMessage)]
    assert [message.name for message in tool_results] == ["find_nearby_places", "calculate_distance"]
