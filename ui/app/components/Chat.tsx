...
function errorText(error: unknown): string {
  ...
  if (status === 429)
    return "You've reached the message limit. Please wait a minute and try again.";
  ...
  if (name === "AgentExecutionError")
    return "The assistant couldn't complete its response. Please try again.";
...
export default function Chat({
  ...
  const visible = stream.messages
    .map(publicMessage)
    .filter((message) => message !== null && message.content);
  const itinerary = threadId ? stream.values.itinerary : null;
  const placeData = usePlaces(
    visible
  ...
  useEffect(() => {
    try {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- Restore browser storage after hydration; server and initial client output must match.
  ...
  useEffect(() => {
    if (followNewest.current && messages.current)
      ...
      await stream.submit(
        { messages: [message] },
        {
          ...
          optimisticValues: (values) => ({
            messages: [
              ...(values.messages ?? []).filter((existing) => existing.id !== message.id),
              message,
            ],
          }),
        },
  ...
  if (!ready)
    return (
      <p className="py-4 text-body-secondary" role="status">
  ...
  const placeDetails = (
    <PlaceDetails
      place={selectedPlaceId ? placeData.places[selectedPlaceId] : undefined}
      ...
      <section className="chat" aria-label="Conversation">
        <div className="d-flex flex-wrap align-items-center justify-content-between py-3 gap-2">
          <div className="d-flex align-items-center gap-2">
            ...
            >
              {itineraryOpen ? "Hide itinerary" : "View itinerary"}
            </button>
          </div>
        ...
        >
          {!visible.length && !busy && (
            <div className="py-5 px-3 text-center text-body-secondary">
              ...
              <div className="list-group list-group-flush text-start">
                {conversationStarters.map((prompt) => (
                  <div className="col" key={prompt}>
                    <button
                      type="button"
                      className="list-group-item w-100 h-100 text-start p-3"
                      disabled={authExpired || prompt.length > maxMessageLength}
                      onClick={() =>
                        void send({ id: crypto.randomUUID(), type: "human", content: prompt })
                      }
                    >
                      {prompt}
                    </button>
          ...
          {busy &&
            (stream.isThreadLoading || !hasNewAssistantText(stream.messages, responseBaseline)) && (
              <ChatProgress
                key={stream.isThreadLoading ? "history" : "reply"}
                loadingHistory={stream.isThreadLoading}
              />
            )}
        </div>
        {error && (
          <div className="alert alert-danger mb-2" role="alert">
            ...
            {authExpired ? (
              ...
              <button
                ...
                disabled={busy}
                onClick={() =>
                  lastSubmission ? void send(lastSubmission) : window.location.reload()
                }
              >
                Try again
              </button>
        ...
        {placeData.error && (
          <div className="alert alert-warning py-2" role="status">
            {placeData.error}{" "}
            <button className="btn btn-sm btn-link" onClick={placeData.retry}>
              Retry place details
            </button>
          </div>
        )}
        ...
        <form onSubmit={submit} className="composer border-top pt-3">
          ...
          <div className="d-flex gap-2 align-items-stretch">
            ...
            <button
              ...
              disabled={busy || authExpired || !input.trim()}
            >
              Send
            </button>
          </div>
