...
export default function ItineraryDays({ itinerary, places, onSelectPlace = () => {} }: {
  itinerary: Itinerary; places: Record<string, Place | undefined>; onSelectPlace?: (placeId: string) => void;
}) {
  ...
  return <div className="itinerary-days">
    {itinerary.days.map((day) => {
      ...
      return <section key={day.day} className="itinerary-day" aria-label={`Day ${day.day}`}>
        <h3 className="h6 mb-3">Day {day.day}{day.title ? ` — ${day.title}` : ""}</h3>
        {!day.stops.length && <p className="small text-body-secondary mb-0">No stops planned yet.</p>}
        <ol className="itinerary-stops">
          {day.stops.map((stop, index) => {
            const place = places[stop.place_id];
            return <li key={`${stop.place_id}:${index}`}>
              {stop.start_time && <div className="small text-body-secondary">{stop.start_time}{stop.end_time ? `–${stop.end_time}` : ""}</div>}
              {place ? <PlaceReference place={place} onSelect={onSelectPlace}>{place.name}</PlaceReference>
                : <span className="text-body-secondary">{Object.hasOwn(places, stop.place_id) ? "Place details unavailable" : "Loading place…"}</span>}
              {stop.reason && <p className="small mb-1 mt-1">{stop.reason}</p>}
              {stop.warnings?.map((warning, index) => <p key={index} className="small text-body-secondary mb-1">⚠️ {warning}</p>)}
            </li>;
          })}
        </ol>
        {pending && <p className="small text-body-secondary" role="status">Loading Day {day.day} places…</p>}
