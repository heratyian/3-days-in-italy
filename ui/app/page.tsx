...
export default async function Home() {
  ...
  return <main className="app-shell container px-3 px-sm-4">
    ...
    {session
      ? <Chat sessionId={session.id} maxMessageLength={positiveInteger(process.env.MAX_MESSAGE_LENGTH, 10000)} />
      : <Login usernameRequired={Boolean(process.env.TEST_AUTH_USERNAME)} configured={Boolean(process.env.TEST_AUTH_PASSWORD)} />}
  </main>;
}
