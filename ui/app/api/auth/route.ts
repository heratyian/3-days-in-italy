...
export async function POST(request: Request) {
  ...
  if (!allowRequest(`login:${address}`, 10)) return errorResponse("Too many attempts. Try again in a minute.", 429);
  let body;
  try {
  ...
  if (!body || typeof body.password !== "string" || !equalSecrets(body.password, password)
