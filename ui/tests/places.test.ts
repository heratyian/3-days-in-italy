...
const render = (content: string) =>
  renderToStaticMarkup(createElement(Message, { human: false, content, places }));
...
test("follow-up suggestions hide bare and comma-separated IDs and preserve punctuation", () => {
  const records = Object.fromEntries(
    [
      ["place_035", "Chianti Day Trip by Bike"],
      ["place_029", "Buca Mario"],
      ["place_033", "Buca dell'Orafo"],
      ["place_040", "San Miniato al Monte"],
  ...
  const html = renderToStaticMarkup(
    createElement(Message, { human: false, content, places: records }),
  );
  assert.doesNotMatch(html, /place_\d/);
  ...
  assert.match(
    render("(**Vatican Museums**, place_010)"),
    /<strong>Vatican Museums<\/strong><\/button>\)/,
  );
...
test("unknown, mismatched and malformed references remain safe prose", () => {
  const html = render(
    "Visit Mystery Museum (place_999). Wrong Museum (place_010). Broken (place_nope).",
  );
...
test("code stays literal, existing links don't contain buttons, and human text is unchanged", () => {
  ...
  const human = renderToStaticMarkup(
    createElement(Message, { human: true, content: "Vatican Museums (place_010)" }),
  );
...
test("details show source facts and secure Maps links with named places", () => {
  ...
  assert.equal(
    new URL(getPlaceMapUrl(place, "google")).searchParams.get("query"),
    buildPlaceQuery(place),
  );
  assert.equal(
    new URL(getPlaceMapUrl({ ...place, latitude: null }, "google")).searchParams.get("query"),
    buildPlaceQuery(place),
  );
  const missing = renderToStaticMarkup(
    createElement(PlaceDetails, {
      place: { ...place, booking_required: null, opening_hours: null, price_range: null },
      onClose() {},
    }),
  );
