...
test("itinerary projection retains traveler details without exposing extra state", () => {
  assert.deepEqual(publicItinerary({ days: [{ day: 1, title: "Rome", secret: "hidden", stops: [{
    place_id: museum.id, start_time: "09:00", end_time: "11:00", reason: "Art and history",
    warnings: ["Reserve tickets", 42], internal: "hidden",
  }] }] }), { days: [{ day: 1, title: "Rome", stops: [{
    place_id: museum.id, start_time: "09:00", end_time: "11:00", reason: "Art and history", warnings: ["Reserve tickets"],
  }] }] });
});

test("itinerary always opens as a modal and stays open as the plan is created, revised, and cleared", { timeout: 5000 }, async () => {
  ...
  try {
    ...
    async function openModal() {
      const shown = new Promise<void>((resolve) => dialog.addEventListener("shown.bs.modal", () => resolve(), { once: true }));
    ...
    itinerary = { days: [{ day: 1, title: "Art morning", stops: [
      { place_id: museum.id, start_time: "09:00", reason: "See the galleries", warnings: ["Book ahead"] },
