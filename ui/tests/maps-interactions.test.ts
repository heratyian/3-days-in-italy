...
test("saved provider overrides platform defaults, including iPads, and storage is optional", () => {
  ...
  try {
    ...
    Object.defineProperty(dom.window, "navigator", {
      configurable: true,
      get() {
        throw new Error("Unavailable platform");
      },
    });
    assert.equal(getPreferredMapProvider(), "google");
  } finally {
...
test("place and structured day actions hydrate safely, share preference, and persist overrides", async () => {
    ...
    days: [
      {
        day: 1,
        title: "Rome",
        stops: [{ place_id: museum.id }, { place_id: "place_999" }, { place_id: neighborhood.id }],
      },
    ],
  };
  function App() {
  ...
  try {
    ...
    assert.match(
      document.querySelector(".itinerary-days")!.textContent!,
      /Unavailable stops are omitted/,
    );
    ...
    assert.match(
      document.querySelector(".itinerary-days a")!.textContent!,
