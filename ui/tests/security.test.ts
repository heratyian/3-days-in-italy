...
test("mutations require a matching browser origin", () => {
  assert.equal(
    sameOrigin(
      new Request("https://test.example/api/auth", { headers: { origin: "https://evil.example" } }),
    ),
    false,
  );
  ...
  assert.equal(
    sameOrigin(
      new Request("https://test.example/api/auth", { headers: { origin: "https://test.example" } }),
    ),
    true,
  );
});
...
test("only one bounded human message is accepted; privileged fields are discarded", () => {
  ...
  assert.deepEqual(
    readUserMessage(
      {
        input: { messages: [message], preferences: { budget: "luxury" }, itinerary: { days: [] } },
        command: {},
      },
      10,
    ),
    ...
    publicValues({
      messages: [
        ...
        { type: "system", content: "secret prompt" },
        {
          ...
          response_metadata: { secret: "hidden" },
        },
      ],
      preferences: { private: true },
    }),
    { messages: [{ type: "ai", content: "Hello" }], itinerary: null },
  );
});
