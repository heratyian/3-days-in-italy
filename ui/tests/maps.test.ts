...
import { buildPlaceQuery, getDayRouteMapUrl, getPlaceMapUrl, getPreferredMapProvider, splitDayRoute } from "../lib/maps";
...
const stops = ["Colosseum", "Roman Forum", "Campo de' Fiori", "Trastevere"].map((name, index) => ({
  ...museum, id: `place_00${index + 1}`, name, neighborhood: null,
}));
const itinerary: Itinerary = { days: [{ day: 1, title: "Ancient Rome", stops: stops.map((place) => ({ place_id: place.id })) }] };
...
test("named place queries use source location fields and coordinates only as fallback", () => {
  ...
  assert.equal(buildPlaceQuery({ ...museum, name: "A & B / café?" }), "A & B / café?, Borgo, Rome, Lazio");
});
...
test("Google routes preserve endpoints and ordered waypoints, with walking mode", () => {
  ...
  assert.deepEqual(four.searchParams.get("waypoints")!.split("|"), stops.slice(1, 3).map(buildPlaceQuery));
...
test("Apple unified directions preserve ordered repeated waypoints in driving mode", () => {
  ...
  assert.equal(new URL(getDayRouteMapUrl(stops.slice(0, 2), "apple")!).searchParams.has("waypoint"), false);
});

test("routes skip unknown records, preserve repeated stops, and hide insufficient days", () => {
  const day = { day: 1, title: null, stops: [{ place_id: museum.id }, { place_id: "place_999" }, { place_id: neighborhood.id }, { place_id: museum.id }] };
  ...
  const html = renderToStaticMarkup(createElement(ItineraryDays, { itinerary: { days: [day] }, places: { [museum.id]: museum, [neighborhood.id]: undefined, place_999: undefined } }));
  // The repeated museum is deliberately preserved, so this day still has two stops.
  assert.match(html, /Open day in Google Maps/);
  const empty = renderToStaticMarkup(createElement(ItineraryDays, { itinerary: { days: [{ ...day, stops: [{ place_id: museum.id }] }] }, places: { [museum.id]: museum } }));
...
test("long days split into connected parts without exceeding mobile or URL limits", () => {
  ...
  for (const provider of ["apple", "google"] as const) {
    ...
    assert.deepEqual(parts.map((part) => part.length), [5, 5, 2]);
    assert.deepEqual(parts.flatMap((part, i) => i ? part.slice(1) : part), longDay);
    ...
    assert.deepEqual(shortParts.flatMap((part, i) => i ? part.slice(1) : part), verbose);
...
test("public state exposes only the saved route projection on streaming and history paths", () => {
  const source = { ...itinerary, secret: "hidden", days: itinerary.days.map((day) => ({ ...day, private: "hidden", stops: day.stops.map((stop) => ({ ...stop, secret: "hidden" })) })) };
  assert.deepEqual(publicValues({ messages: [], itinerary: source, preferences: { private: true } }), { messages: [], itinerary });
  assert.equal(publicItinerary({ days: [{ day: 1, stops: "invalid" }] }), null);
  assert.equal(publicItinerary(null), null);
  assert.deepEqual(publicItinerary({ days: [{ day: 1, stops: [null, {}, { place_id: "invalid" }, { place_id: "place_999" }] }] }),
...
test("day links show authoritative stop order, wait for data, and name split parts", () => {
  ...
  const long = { days: [{ ...itinerary.days[0], stops: [...itinerary.days[0].stops, ...itinerary.days[0].stops] }] };
