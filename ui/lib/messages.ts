  ...
  const content =
    typeof message.content === "string"
      ...
      : Array.isArray(message.content)
        ...
        : "";
  return {
    type: type === "human" ? ("human" as const) : ("ai" as const),
    content,
...
export function publicValues(value: unknown) {
  ...
  const itinerary =
    value && typeof value === "object" && "itinerary" in value
      ? publicItinerary(value.itinerary)
      : null;
  return {
    messages: Array.isArray(messages)
      ? messages.map(publicMessage).filter((message) => message !== null)
      : [],
    itinerary,
  };
}
...
export function hasNewAssistantText(messages: unknown[], previousMessages: unknown[]): boolean {
  const previous = new Map(
    previousMessages.map((value, index) => {
